...
// navigate function - shows and hides sections based on the id parameter
const navigate = (id) => {
  console.log(id);

  id = id === "" ? "home" : id;

  // get all div class='content-item'
  const elems = document.querySelectorAll(".content-item");

  // loop above divs and hide all
  elems.forEach((x) => {
    x.classList.add("hide");
  });

  // show the div from id
  document.getElementById(id).classList.remove("hide");

  // make the browser change the url to /{id}
  history.pushState({}, "", "/" + id === "home" ? "" : id);
};

// this will run on window load to navigate to the selected route in the browser
window.addEventListener("load", () => {
  let url = document.location.href; // returns https://localhost:3000/{id}
  let splitUrl = url.split("/"); // returns ['https', '', 'localhost:3000', '{id}'] as an array of strings

  let route = splitUrl[splitUrl.length - 1]; // gets last item from splitUrl - {id}
  console.log(url, splitUrl, route);

  if (route.length > 0) {
    // use above navigate function to change url and hide/show sections
    navigate(route);
  }
});
